...
The effect is immediate when either the current GrowthDelay is zero or when the AVN owner calls the function (passing empty an t2TransactionId and confirmations); the amount is minted, locked in the AVN, and the following event is emitted: _**LogGrowth(uint256 indexed amount, uint32 indexed period)**_
