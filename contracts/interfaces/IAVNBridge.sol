...
interface IAVNBridge {
  ...
  function triggerGrowth(uint128 amount, uint32 period, uint64 expiry, uint32 t2TransactionId, bytes calldata confirmations)
      external;

  // Validators only
  function registerValidator(bytes calldata t1PublicKey, bytes32 t2PublicKey, uint64 expiry, uint32 t2TransactionId,
      bytes calldata confirmations) external;
  function deregisterValidator(bytes calldata t1PublicKey, bytes32 t2PublicKey, uint64 expiry, uint32 t2TransactionId,
      bytes calldata confirmations) external;
  function publishRoot(bytes32 rootHash, uint64 expiry, uint32 t2TransactionId, bytes calldata confirmations) external;
