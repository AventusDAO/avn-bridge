...
describe('Owner Functions', () => {
  ...
  context('setClaimedLowers', () => {
    ...
    it('owner can set IDs in a single bucket', async () => {
      ...
      for (const id of ids) {
        expect(await bridge.lowerClaimed(id)).to.equal(true);
      }

      for (const id of [0, 3, 5, 7, 10, 11, 12]) {
        expect(await bridge.lowerClaimed(id)).to.equal(false);
      }
    });
    ...
    it('owner can set IDs in multiple buckets including IDs on the boundary', async () => {
      ...
      await bridge.setClaimedLowers([0, 1], [bucket0, bucket1]);

      expect(await bridge.lowerClaimed(255)).to.equal(true);
      expect(await bridge.lowerClaimed(256)).to.equal(true);
      expect(await bridge.lowerClaimed(259)).to.equal(true);

      for (const id of [254, 257, 258, 260]) {
        expect(await bridge.lowerClaimed(id)).to.equal(false);
      }
    });
    ...
    it('buckets can be overwritten', async () => {
      ...
      expect(await bridge.lowerClaimed(1)).to.equal(true);
      expect(await bridge.lowerClaimed(2)).to.equal(true);
      expect(await bridge.lowerClaimed(4)).to.equal(true);

      const second = toWord([2]);
      await bridge.setClaimedLowers([0], [second]);
    ...
    it('handles unsorted bucket indices', async () => {
      const bucket0 = toWord([1, 3]); // IDs 1,3
      const bucket2 = toWord([10]); // ID 522
      const bucket1 = toWord([0, 255]); // IDs 256, 511

      await bridge.setClaimedLowers([2, 0, 1], [bucket2, bucket0, bucket1]);

      expect(await bridge.lowerClaimed(1)).to.equal(true);
      expect(await bridge.lowerClaimed(3)).to.equal(true);
      expect(await bridge.lowerClaimed(256)).to.equal(true);
      expect(await bridge.lowerClaimed(511)).to.equal(true);
      expect(await bridge.lowerClaimed(522)).to.equal(true);

      expect(await bridge.lowerClaimed(2)).to.equal(false);
      expect(await bridge.lowerClaimed(510)).to.equal(false);
      expect(await bridge.lowerClaimed(521)).to.equal(false);
    });

    it('last write wins when the same bucket appears multiple times in one call', async () => {
      const first = toWord([1, 2, 4]);
      const second = toWord([2]);

      await bridge.setClaimedLowers([0, 0], [first, second]);
    ...
    it('supports high lower IDs', async () => {
      const id = 1234567;
      const bucket = Math.floor(id / 256);
      const bit = BigInt(id % 256);
      const word = 1n << bit;

      await bridge.setClaimedLowers([bucket], [word]);
    ...
    it('bulk set ~7k lowers with single bit (bit 23) cleared per bucket', async () => {
      ...
      const wordAllBut23 = allOnes ^ (1n << CLEARED_BIT);

      const buckets = Array.from({ length: NUM_BUCKETS }, (_, i) => START_BUCKET + i);
      const words = Array.from({ length: NUM_BUCKETS }, () => wordAllBut23);

      const tx = await bridge.setClaimedLowers(buckets, words);
      const receipt = await tx.wait();
      console.log('Gas cost to set ~7k claimed lowers =', receipt.gasUsed.toString());

      for (let i = 0; i < NUM_BUCKETS; i++) {
        const bucket = START_BUCKET + i;
        const idCleared = bucket * 256 + Number(CLEARED_BIT);
        const idLeft = idCleared - 1;
        const idRight = idCleared + 1;

        expect(await bridge.lowerClaimed(idCleared)).to.equal(false);
        expect(await bridge.lowerClaimed(idLeft)).to.equal(true);
        expect(await bridge.lowerClaimed(idRight)).to.equal(true);
      }
    });
    ...
    context('fails', () => {
      it('when the caller is not the owner', async () => {
        ...
        await expect(bridge.connect(someOtherAccount).setClaimedLowers([0], [word])).to.be.revertedWith('Ownable: caller is not the owner');
      });

      it('when array lengths mismatch', async () => {
        await expect(bridge.setClaimedLowers([0, 1], [1n])).to.be.revertedWithoutReason();
      });
    });
  });
