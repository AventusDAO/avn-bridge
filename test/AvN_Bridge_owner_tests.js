  ...
  context('migrate', () => {
    ...
    it('owner can set IDs in a single bucket', async () => {
      ...
      for (const id of [0, 3, 5, 7, 10, 11, 12]) {
        expect(await bridge.lowerUsed(id)).to.equal(false);
      }
    });
    ...
    it('owner can set IDs in multiple buckets including IDs on the boundary', async () => {
      ...
      expect(await bridge.lowerUsed(259)).to.equal(true);
    ...
    it('buckets can be overwritten', async () => {
      ...
      expect(await bridge.lowerUsed(4)).to.equal(false);
    });
    ...
    it('handles unsorted bucket indices', async () => {
      ...
      expect(await bridge.lowerUsed(256)).to.equal(true);
      expect(await bridge.lowerUsed(511)).to.equal(true);
      expect(await bridge.lowerUsed(522)).to.equal(true);

      expect(await bridge.lowerUsed(2)).to.equal(false);
      expect(await bridge.lowerUsed(510)).to.equal(false);
      expect(await bridge.lowerUsed(521)).to.equal(false);
    });
    ...
    it('bulk set ~7k lowers with single bit (bit 23) cleared per bucket', async () => {
      ...
      for (let i = 0; i < NUM_BUCKETS; i++) {
        ...
        expect(await bridge.lowerUsed(idCleared)).to.equal(false);
        expect(await bridge.lowerUsed(idLeft)).to.equal(true);
        expect(await bridge.lowerUsed(idRight)).to.equal(true);
      }
    });
