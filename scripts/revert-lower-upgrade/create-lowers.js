const { init, sleep } = require('./common');
...
async function sendBatch(t2Api, t2Signer, tokenAddress, start, count) {
  const calls = [];
  for (let i = 0; i < count; i++) {
    calls.push(t2Api.tx.tokenManager.scheduleDirectLower(t2Signer.address, tokenAddress, start + i, T1_RECIPIENT));
  }

  return new Promise(async (resolve, reject) => {
    const batchTx = t2Api.tx.utility.batch(calls);
...
async function main() {
  ...
  try {
    const tokenAddress = (await t2Api.query.tokenManager.avtTokenContract()).toString();
    ...
    while (created < MAX_LOWERS) {
      ...
      const lowerIds = await sendBatch(t2Api, t2Signer, tokenAddress, amount, batch);
